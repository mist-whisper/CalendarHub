/**
 * src/competitions/euro/fetcher.js
 * 欧洲杯数据抓取与标准清洗模块
 */
...
const STAGE_MAP = {
  ...
  'SEMI_FINALS': '半决赛',
  'FINAL': '决赛'
  // 注：欧洲杯自 1984 年起取消了三四名决赛 (THIRD_PLACE)
};
...
async function fetchMatches(config) {
  ...
  const competitionCode = config.competitionCode || 'EC'; // 欧洲杯代码为 EC (European Championship)
  ...
  console.log(`[EuroFetcher] 正在向 API 请求 ${competitionCode} 赛程...`);
  const data = await fetchJsonWithRetry(url, { headers });

  if (!data || !data.matches || !Array.isArray(data.matches)) {
    console.warn('[EuroFetcher] 未能获取到有效的欧洲杯比赛列表');
    return [];
  }
  ...
  return data.matches.map(match => {
    ...
    // 2. 赛事阶段翻译
    const stage = STAGE_MAP[match.stage] || match.stage || '欧洲杯';
